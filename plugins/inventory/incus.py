...
class InventoryModule(BaseInventoryPlugin, Cacheable, Constructable):
    ...
    def load_yaml_data(self, filepath):
        try:
            ...
            raise AnsibleParserError(
                "Could not load the test data from {0}: {1}".format(
                    to_native(filepath), to_native(err)
                )
            )
    ...
    def verify_file(self, path):
        ...
        self.display.vvv(
            f"{path} is not a valid file or does not end with incus.yml or incus.yaml"
        )
        return False
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        try:
            ...
            raise AnsibleParserError(
                f"Missing required configuration parameter: {kerr}"
            )
    ...
    def _cleandata(self):
        ...
        def keep_instance(instance):
            ...
            if (
                ...
                and instance["state"]["status"].lower() != self.state_filter
            ):
                return False
            return True
    ...
    def _get_interface(self, instance):
        ...
        for name in network.keys():
            ...
            for adr in network[name]["addresses"]:
                ...
                if (
                    adr["scope"] == "global"
                    and adr["family"] == self.prefered_instance_network_family
                ):
                    adr["name"] = name
                    return adr
    ...
    def _build_group_from_var(self, var_name, group_prefix):
        for instance_name in self.inventory.hosts:
            if var_name in self.inventory.get_host(instance_name).get_vars():
                group_name = (
                    self.inventory.get_host(instance_name).get_vars().get(var_name)
                )
                if group_prefix:
    ...
    def _build_group_from_var_equals(self, group_name, var_name, want):
        ...
        for instance_name in self.inventory.hosts:
            if var_name in self.inventory.get_host(instance_name).get_vars():
                ...
                _t = type(got)
                match _t:
                    case _s if issubclass(_t, str):
                        if want in got.lower():
                            self.inventory.add_child(group_name, instance_name)
                    case _l if issubclass(_t, list):
                        if want in got:
                            self.inventory.add_child(group_name, instance_name)
                    case _:
        ...
        if self.groupby:
            for group_name in self.groupby:
                if not group_name.isalnum():
                    raise AnsibleParserError(
                        "Invalid character(s) in groupname: {0}".format(
                            to_native(group_name)
                        )
                    )
                ...
                if group_type not in groupmap:
                    raise AnsibleParserError(
                        "Invalid group type: {0}".format(to_native(group_type))
                    )
                ...
                if group_value:
                    self._build_group_from_var_equals(
                        group_name, group_var, group_value
                    )
                else:
    ...
    def build_inventory_hosts(self):
        for instance in self.data:
            ...
            if self.type_filter != "all" and instance_type != self.type_filter:
                self.display.v(
                    f'Instance "{instance_name}" excluded, type "{instance_type}" does not match filter'
                )
                continue

            if self.project_filter != "all" and instance_project != self.project_filter:
                self.display.v(
                    f'Instance "{instance_name}" excluded, project "{instance_project}" does not match filter'
                )
                continue
            if self.status_filter != "all" and instance_status != self.status_filter:
                self.display.v(
                    f'Instance "{instance_name}" excluded, status "{instance_status}" does not match filter'
                )
                continue
            # TODO: more filtering
            ...
            if instance_type != "container" and not iface:
                self.display.warning(
                    f'Instance "{instance_name}" excluded, has no network interface and is type "{instance_type}"'
                )
                continue

            self.inventory.add_host(instance_name)
            self.inventory.set_variable(
                instance_name, "ansible_incus_type", instance_type
            )
            self.inventory.set_variable(
                instance_name, "ansible_incus_project", instance_project
            )
            self.inventory.set_variable(
                instance_name, "ansible_incus_status", instance_status
            )
            self.inventory.set_variable(
                instance_name, "ansible_incus_profiles", instance["profiles"]
            )
            self.inventory.set_variable(
                instance_name, "ansible_incus_location", instance["location"]
            )
            self.inventory.set_variable(
                instance_name, "ansible_incus_architecture", instance["architecture"]
            )

            self.inventory.set_variable(
                instance_name,
                "ansible_incus_os",
                self._get_config_value(instance, "image.os"),
            )
            self.inventory.set_variable(
                instance_name,
                "ansible_incus_release",
                self._get_config_value(instance, "image.release"),
            )

            if iface:
                self.inventory.set_variable(
                    instance_name, "ansible_host", iface["address"]
                )
                # FIXME: vlanid
                self.inventory.set_variable(
                    instance_name, "ansible_incus_vlan_ids", None
                )

            if instance["type"] == "container":
                self.inventory.set_variable(
                    instance_name, "ansible_host", instance_name
                )
                self.inventory.set_variable(
                    instance_name, "ansible_connection", "community.general.incus"
                )
            elif iface:
