...
class IncusNetworkManagement(object):
    def __init__(self, module, **kwargs):
        ...
        try:
            self.client = IncusClient(
                project=self.project, target=self.target, debug=self.debug
            )
    ...
    def _is_managed(self):
        return self.current_network.get("type", "") == "sync" and bool(
            self.current_network.get("metadata", {}).get("managed", False)
        )
    ...
    def _delete_network(self):
        if not self._is_managed():
            self.module.fail_json(
                msg="Failed to remove network. It is not managed by incus"
            )
            return
        ...
        # update diff
        self.diff["after"]["network"] = dict(
            name=self.name, project=self.project, state="absent"
        )
    ...
    def _created(self):
        if self.current_state == "unmanaged":
            self.module.fail_json(
                msg="Failed to (re)create network. It exists but is not managed by incus"
            )
            return
    ...
    def _deleted(self):
        ...
        if self.current_state == "unmanaged":
            self.module.fail_json(
                msg="Failed to delete network. It exists but is not managed by incus"
            )
            return
