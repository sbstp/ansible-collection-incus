...
class IncusProfileManagement(object):
    ...
    def _update(self):
        ...
        got = json.dumps(
            clean_resource(self.current, state=self.current_state), sort_keys=True
        )
        ...
        if got != want:
            if not self.module.check_mode:
                self.client.update_profile(
                    self.name, self.description, self.config, self.devices
                )
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            state=dict(
                type="str", choices=["present", "updated", "absent"], default="present"
            ),
        ),
